...
<div class="mb-6 flex justify-between items-center">
  <div>
    <h2 class="text-2xl font-bold text-gray-900">Info Akademik</h2>
    <p class="text-gray-600 text-sm mt-1">Informasi jadwal kuliah dan akademik.</p>
  </div>
...
{#if showForm}
  <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-8 animate-fade-in-up">
    ...
    <form method="POST" action="?/create" use:enhance class="space-y-4">
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Judul Info</label>
      ...
      <div>
        ...
        <select name="type" required class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-dark focus:border-dark">
          <option value="jadwal">jadwal</option><option value="info">info</option><option value="lainnya">lainnya</option>
        </select>
      </div>
      <div class="pt-2">
  ...
  <div class="overflow-x-auto">
    <table class="w-full text-left text-sm">
      <thead class="bg-gray-50 text-gray-600 border-b border-gray-100">
        <tr>
          <th class="px-6 py-4 font-semibold">Judul</th><th class="px-6 py-4 font-semibold">Tipe</th>
      ...
      <tbody class="divide-y divide-gray-50">
        {#each data.items as item}
          <tr class="hover:bg-gray-50/50 transition-colors">
            <td class="px-6 py-4 font-medium text-gray-900">{item.title}</td><td class="px-6 py-4 font-medium text-gray-900">{item.type}</td>
